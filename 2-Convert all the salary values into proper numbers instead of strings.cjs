...
let updatedSalaryArr = data.map((eachEmployee) => {
    if (typeof eachEmployee.salary == 'string'){
        let newSalary = parseInt(eachEmployee.salary.slice(1,eachEmployee.salary.length))
...
// for (let each of updatedSalaryArr) {
//     console.log(typeof each.salary, each.salary)
// }